...
thickness styles, line types, point types
...
ggplot(horseshoe, aes(x = width, y = weight)) + 
  geom_point() + 
  facet_wrap(vars(color)) # (vars is just like aes, looks inside the data set)
```
...
\includegraphics[width=3cm]{colour_scales.jpg}
...
- Use `width` and `height` arguments to vary hte amount of jittering
...
Draw the ugliest plot you can using the commands we have covered above! Submit it to Slack. Like other people's graphs to find the winner 
